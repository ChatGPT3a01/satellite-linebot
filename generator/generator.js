...
function validateForm() {
    const requiredFields = ['lineToken', 'openweatherKey', 'stormglassKey', 'sheetId'];
    let isValid = true;
...
function handleGenerate() {
    ...
    generateBtn.textContent = '正在生成程式碼...';

    // 模擬載入延遲
    setTimeout(() => {
        const config = {
            lineToken: document.getElementById('lineToken').value,
            openweatherKey: document.getElementById('openweatherKey').value,
            stormglassKey: document.getElementById('stormglassKey').value,
            sheetId: document.getElementById('sheetId').value
        };

        generatedCode = generateGASCode(config);
        ...
        generateBtn.textContent = '🚀 生成 GAS 程式碼';
...
const CONFIG = {
  // Line Bot 設定
  LINE_CHANNEL_ACCESS_TOKEN: '${config.lineToken}',

  // OpenWeatherMap 設定 (https://openweathermap.org/api)
  OPENWEATHER_API_KEY: '${config.openweatherKey}',

  // Stormglass 設定 (https://stormglass.io/)
  STORMGLASS_API_KEY: '${config.stormglassKey}',
